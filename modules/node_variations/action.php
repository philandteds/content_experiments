...
} elseif( $http->hasVariable( 'UpdateExperimentCode' ) ) {
	$code = $http->hasVariable( 'ExperimentCode' ) ? $http->variable( 'ExperimentCode' ) : null;
	$experimentCode = ExperimentCode::fetchByNodeID( $node->attribute( 'node_id' ) );
	$experimentCode = $experimentCode['result'];
	if( $experimentCode instanceof ExperimentCode === false ) {
		$experimentCode = new ExperimentCode( array( 'node_id' => $node->attribute( 'node_id' ) ) );
	}
	$experimentCode->setAttribute( 'experiment_code', $code );
	$experimentCode->store();
}
